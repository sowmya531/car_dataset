...
1.Find all the Null Values in the dataset. If there are any null values, fill them with the mean of that respective column using isnull() and fillna().
2.Check the different types of 'Make' available in the dataset and their count using value_counts().
3.Show all the records where 'Origin' is either Asia or Europe using isin().
4.Remove all the records (rows) where the 'Weight' is above 4000.
5.Increase all the values of the 'MPG_City' column by 3 using apply().
